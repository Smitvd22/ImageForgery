...
def create_filename_blind_test():
    ...
    # Sample some files from each class
    authentic_samples = df[df['label'] == 0].sample(n=5, random_state=42)
    forged_samples = df[df['label'] == 1].sample(n=5, random_state=42)
...
def run_blind_predictions(test_cases):
    ...
    predictor = OptimizedPredictor(dataset='misd')
